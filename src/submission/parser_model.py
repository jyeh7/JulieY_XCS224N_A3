...
class ParserModel(nn.Module):
    ...
    def reset_parameters(self) -> None:
        ...
        # Initialize `self.embed_to_hidden` linear layer's weight matrix
        nn.init.xavier_uniform_(self.embed_to_hidden.weight)
        # Initialize `self.hidden_to_logits` linear layer's weight matrix
    ...
    def embedding_lookup(self, t):
        ...
        # Use `self.pretrained_embeddings` to lookup the embeddings for the input tokens in `t`
        x = self.pretrained_embeddings(t) # tensor shape: (batch_size, n_features, embed_size)
        # Use the tensor `view` method to reshape the embeddings tensor to (batch_size, n_features * embedding_size)
        batch_size = x.size(0)
    ...
    def forward(self, t):
        ...
        # Apply `self.embedding_lookup` to `t` to get the embeddings
        x = self.embedding_lookup(t)

        # Apply `embed_to_hidden` linear layer to the embeddings
        x = self.embed_to_hidden(x)
        
        # Apply relu non-linearity to the output of step 2 to get the hidden units
        x = torch.relu(x)

        # Apply dropout layer to the output of step 3.
        x = self.dropout(x)

        # Apply `hidden_to_logits` layer to the output of step 4 to get the logits
