...
def minibatch_parse(sentences, model, device, batch_size):
    ...
    while unfinished_parses:
        # Take the next batch_size parses in unfinished.parses as a minibatch
        minibatch = unfinished_parses[:batch_size]

        # Predict the next transition for each partial parse in the minibatch
        predicted_transitions = model.predict(minibatch, device=device)

        # Perform a parse step on each partial parse in the minibatch with its predicted transition
        for parse, predicted_transition in zip(minibatch, predicted_transitions):
            parse.parse_step(predicted_transition)

        # Remove the completed (empty buffer and stack of size 1) parses from unfinished.parses
        unfinished_parses = [parses for parses in unfinished_parses if not (len(parses.buffer) == 0 and len(parses.stack) == 1)]
    
    # Dependencies for each (now completed) parse in partial.parses
